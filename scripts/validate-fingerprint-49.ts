...
const REGION_FENCING_SIGNALS: { pattern: RegExp; region: string; severity: "high" | "medium" }[] = [
  // Timezone restrictions
  { pattern: /\bUTC[-+]\d+\s+to\s+UTC[-+]?\d+\b/i, region: "timezone_range", severity: "high" },
  { pattern: /\bGMT[-+]\d+\s+to\s+GMT[-+]?\d+\b/i, region: "timezone_range", severity: "high" },
  { pattern: /\bworking\s+hours.*(?:UTC|GMT|EST|PST|CET|EET|IST|JST|AEST)/i, region: "timezone_named", severity: "high" },
  { pattern: /\btimezone.*(?:UTC|GMT|EST|PST|CET|EET|IST|JST|AEST)/i, region: "timezone_named", severity: "high" },
  { pattern: /\b(?:within|during)\s+(?:our|the)\s+(?:business|core)\s+hours\b/i, region: "business_hours", severity: "medium" },
  { pattern: /\boverlap\s+(?:with|of)\s+\d+\s+hours?\b/i, region: "overlap_requirement", severity: "medium" },
  // Region-named restrictions
  { pattern: /\b(?:EMEA|APAC|Latam|Americas|Europe|Asia|Africa)\s+(?:hours|timezone|time)\b/i, region: "region_hours", severity: "high" },
  { pattern: /\bmust\s+(?:be\s+)?(?:based|located)\s+in\s+(?:Europe|Asia|Africa|South\s+America|North\s+America|EMEA|APAC|Latam)\b/i, region: "region_must_be", severity: "high" },
  { pattern: /\bcandidates\s+(?:from|in|based\s+in)\s+(?:Europe|Asia|Africa|South\s+America|North\s+America|EMEA|APAC|Latam)\b/i, region: "region_candidates", severity: "high" },
  // Country-specific work authorization
  { pattern: /\beligible\s+to\s+work\s+(?:in|for)\s+(?:the\s+)?(?:US|USA|UK|EU|Germany|France|India|Poland|Brazil|Canada|Australia|Singapore)\b/i, region: "work_auth", severity: "high" },
  { pattern: /\bwork\s+(?:authorization|permit|visa)\s+(?:required|needed)\s+(?:in|for|to\s+work\s+in)\s+/i, region: "work_auth", severity: "high" },
  // Salary currency indicators (strong region signal)
  { pattern: /\bsalary.*(?:EUR|GBP|PLN|INR|BRL|CAD|AUD|SGD|ZAR|AED)\b/i, region: "salary_currency", severity: "medium" },
  { pattern: /\bcompensation.*(?:EUR|GBP|PLN|INR|BRL|CAD|AUD|SGD|ZAR|AED)\b/i, region: "salary_currency", severity: "medium" },
  // Specific country/region in JD text (not location field)
  { pattern: /\bonly\s+(?:accepting|considering|hiring)\s+(?:candidates\s+)?(?:from|in)\s+/i, region: "only_from", severity: "high" },
  { pattern: /\bmust\s+(?:reside|live|be\s+located)\s+in\s+/i, region: "must_reside", severity: "high" },
];
...
const GLOBAL_SIGNALS: { pattern: RegExp; strength: "high" | "medium" }[] = [
  ...
  { pattern: /\bteam\s+members\s+across\s+\d+\s+countries\b/i, strength: "high" },
  { pattern: /\bfully\s+remote\b/i, strength: "medium" },
  { pattern: /\b100\s?%\s+remote\b/i, strength: "medium" },
  { pattern: /\bdistributed\s+(?:team|company|workforce)\b/i, strength: "medium" },
];
...
function analyzeJd(text: string): {
  ...
  const fencingScore = fencingSignals.filter(s => s.severity === "high").length * 2 +
    fencingSignals.filter(s => s.severity === "medium").length;
  const globalScore = globalSignals.filter(s => s.strength === "high").length * 2 +
...
async function main() {
  console.log("=== Fingerprint Validation: Are the 49 Genuinely Worldwide? ===\n");
  ...
  for (const c of ashbyCompanies) {
    ...
    for (const j of jobs) {
      ...
      if (isRemote(j.workplace_type, loc) && !isFenced(loc) && isMainstream(j.title)) {
        addressableJobs++;
      }
    }
  ...
  for (const c of ashbyCompanies) {
    ...
    // Score the company
    const avgFencingScore = jobs.length > 0 ? totalFencingScore / jobs.length : 0;
    ...
    if (avgFencingScore >= 1.0 && fencingRate >= 0.3) {
      ...
      likelyRegionFenced++;
    } else if (avgFencingScore >= 0.5 || (fencingRate >= 0.15 && avgGlobalScore < 1.0)) {
      verdict = "ambiguous";
  ...
  const fenced = validations.filter(v => v.verdict === "likely_region_fenced");
  if (fenced.length > 0) {
    console.log("=== LIKELY REGION-FENCED (contamination in fingerprint) ===");
    console.table(fenced.map(v => ({
      slug: v.slug,
      name: v.name,
      jobs: v.jobs,
  ...
  // 8. Summary
  console.log("\n=== SUMMARY ===");
  console.log(`Validated fingerprint: ${genuinelyGlobal} genuinely global companies`);
  console.log(`Contamination removed: ${likelyRegionFenced} likely region-fenced companies`);
  console.log(`Needs manual review: ${ambiguous} ambiguous companies`);
  console.log("");
  console.log("VERDICT:");
  if (likelyRegionFenced === 0 && ambiguous === 0) {
    console.log("  All 49 companies validated as genuinely worldwide — fingerprint is trustworthy.");
  } else if (likelyRegionFenced <= 3) {
    console.log(`  Low contamination (${likelyRegionFenced} region-fenced) — fingerprint is usable with minor cleanup.`);
  } else {
    console.log(`  High contamination (${likelyRegionFenced} region-fenced) — fingerprint needs cleanup before L2 targeting.`);
    console.log("  RECOMMENDATION: Remove region-fenced companies from the reference set before scoring L2 sources.");
  }
