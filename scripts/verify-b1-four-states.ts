...
async function main() {
  ...
  if (state0[0].tier !== "probation") {
    console.error(
      "FAIL: Company is not on probation. Pick a different company.",
    );
    process.exit(1);
  }
  ...
  `;
  console.log(
    "  Company state updated (last_polled_at set, last_job_posted_at set)",
  );
  ...
  console.log(
    "  (This is what jobIngestedHandler does when company.tier = 'probation')",
  );
  ...
  for (const jobId of insertedJobIds) {
    ...
    const normText =
      jobRow[0].title +
      " — Software engineering role. Requires TypeScript, React, Node.js, and GraphQL experience. Full-stack development with modern web technologies.";
    await sql`
      UPDATE job SET
  ...
  console.log(
    `  Normalized ${insertedJobIds.length} jobs (normalized_text set, job_embedding left NULL)`,
  );
  ...
  console.log(
    `  ✓ All ${stateA.length} jobs have job_embedding = NULL: ${allNullA}`,
  );
  console.log(
    `  ✓ All ${stateA.length} jobs have normalized_text populated: ${allNormA}`,
  );
  if (!allNullA || !allNormA) {
    console.error(
      "  FAIL: State A assertion failed — embeddings should be NULL during probation",
    );
    process.exit(1);
  }
  ...
  console.log(
    `  ✓ All ${stateB.length} jobs have job_embedding populated: ${allEmbeddedB}`,
  );
  if (!allEmbeddedB) {
    console.error(
      "  FAIL: State B assertion failed — embeddings should be populated after backfill",
    );
    process.exit(1);
  }
  ...
  const persona =
    await sql`SELECT persona_embedding::text as embedding, persona_label FROM persona LIMIT 1`;
  ...
  console.log(
    `  ✓ ${gate2Results.length} jobs visible in Gate 2 HNSW search: ${visibleInGate2}`,
  );
  if (!visibleInGate2) {
    console.error(
      "  FAIL: State C assertion failed — jobs should be visible in Gate 2 after embedding",
    );
    process.exit(1);
  }
  ...
  `;
  console.log(
    `  Jobs with NULL embedding (invisible to Gate 2): ${nullEmbeddingCount[0].cnt}`,
  );
  console.log(
    `  → Before backfill, all ${stateA.length} jobs were in this state — invisible to Gate 2`,
  );
  ...
  `;
  console.log(
    `  Embeddings before second backfill run: ${embedCountBefore[0].cnt}`,
  );
  ...
  `;
  console.log(
    `  Pending jobs found by second backfill run: ${pendingAfterBackfill.length}`,
  );
  ...
  `;
  console.log(
    `  Embeddings after second backfill run: ${embedCountAfter[0].cnt}`,
  );

  const idempotent =
    pendingAfterBackfill.length === 0 &&
    embedCountBefore[0].cnt === embedCountAfter[0].cnt;
  console.log(
    `  ✓ Second run found 0 pending jobs: ${pendingAfterBackfill.length === 0}`,
  );
  console.log(
    `  ✓ Embedding count unchanged (${embedCountBefore[0].cnt} → ${embedCountAfter[0].cnt}): ${embedCountBefore[0].cnt === embedCountAfter[0].cnt}`,
  );
  console.log(`  ✓ Idempotency proven: ${idempotent}`);

  if (!idempotent) {
    console.error(
      "  FAIL: State D assertion failed — backfill is not idempotent",
    );
    process.exit(1);
  }
  ...
  console.log(
    `  Company: ${ATS_SLUG} (${ATS_SOURCE}) — was on probation, never polled`,
  );
  console.log("");
  console.log("  STATE A: job_embedding = NULL during probation");
  console.log(
    `    → ${stateA.length} jobs with normalized_text populated, job_embedding = NULL`,
  );
  console.log(
    `    → Row evidence: ${stateA.map((r: any) => `${r.id.substring(0, 8)}:embed=NULL`).join(", ")}`,
  );
  console.log("");
  console.log("  STATE B: job_embedding populated after promotion + backfill");
  console.log(
    `    → ${stateB.length} jobs with job_embedding populated (via OpenAI text-embedding-3-small)`,
  );
  console.log(
    `    → Row evidence: ${stateB.map((r: any) => `${r.id.substring(0, 8)}:embed=POPULATED`).join(", ")}`,
  );
  console.log("");
  console.log("  STATE C: Jobs visible in Gate 2 HNSW similarity query");
  console.log(
    `    → ${gate2Results.length} jobs returned by cosine similarity search`,
  );
  console.log(
    `    → Similarity scores: ${gate2Results.map((r: any) => r.similarity.toFixed(3)).join(", ")}`,
  );
  console.log("");
  console.log(
    "  STATE D: Backfill idempotent (run twice, embeds exactly once)",
  );
  console.log(`    → First run: embedded ${embeddedCount} jobs`);
  console.log(
    `    → Second run: found ${pendingAfterBackfill.length} pending jobs (no-op)`,
  );
  console.log(
    `    → Embedding count: ${embedCountBefore[0].cnt} → ${embedCountAfter[0].cnt} (unchanged)`,
  );
  console.log("");
