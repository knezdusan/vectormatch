...
const FENCING_PATTERNS = [
  ...
  {
    pattern: /united kingdom|uk\b|london|manchester|england/i,
    type: "country",
  },
  {
    pattern: /canada|toronto|vancouver|montreal|ottawa|calgary/i,
    type: "country",
  },
  {
    pattern:
...
function classifyJob(job: any): {
  ...
  const addressable =
    isRemote && fencingType === null && (isMainstream || !isNiche);
...
async function main() {
  ...
  for (const source of sources) {
    ...
    for (let i = 0; i < companies.length; i++) {
      ...
      try {
        ...
        for (const job of jobsArray) {
          ...
          if (classification.isRemote && classification.fencingType === null)
            globalRemoteJobs++;
    ...
    console.log(
      `  ${source}: ${hits}/${companies.length} hits, ${addressableCompanies} addressable (${(addressableDensity * 100).toFixed(1)}%)`,
    );
  }
  ...
  const totalBacklog = backlog.reduce(
    (s: number, r: any) => s + Number(r.cnt),
    0,
  );
  const totalAddressable = extrapolation.reduce(
    (s: number, r: any) =>
      s + (typeof r.est_addressable === "number" ? r.est_addressable : 0),
    0,
  );
  console.log(`\n  Total backlog: ${totalBacklog}`);
  console.log(`  Total estimated addressable: ${totalAddressable} companies`);
  console.log(
    `  Overall addressable density: ${((totalAddressable / totalBacklog) * 100).toFixed(1)}%`,
  );

  // 5. Key insight
  console.log("\n=== Key Insight ===");
  console.log(
    "Addressable = has ≥1 job that is remote + NOT country-fenced + mainstream stack.",
  );
  console.log(
    "This is measured from location_name heuristics, NOT the contaminated remote_scope field.",
  );
  console.log(
    "The per-source addressable density is the number that should decide L2 targeting.",
  );
