...
async function main() {
  ...
  for (const c of allCompanies) {
    ...
    for (const j of jobs) {
      ...
      const isMainstream = MAINSTACK_KEYWORDS.some((k) =>
    ...
    if (addressableJobs > 0) {
      addressable.push({
        slug: c.ats_slug,
        source: c.ats_source,
        name: (c.company_name || c.ats_slug)
          .replace(/&amp;/g, "&")
          .substring(0, 30),
        jobs: jobs.length,
        remote: remoteJobs,
        ...
        remote_density:
          jobs.length > 0
            ? Math.round((remoteJobs / jobs.length) * 100) + "%"
            : "0%",
        addr_density:
          jobs.length > 0
            ? Math.round((addressableJobs / jobs.length) * 100) + "%"
            : "0%",
      });
    }
  }
  ...
  console.log(
    "Addressable companies (location-filtered, ATS-agnostic):",
    addressable.length,
    "\n",
  );
  ...
  console.table(
    Object.entries(bySource).map(([s, c]) => ({ source: s, count: c })),
  );
  ...
  console.log(
    "ATS-agnostic: companies from",
    Object.keys(bySource).length,
    "sources",
  );
  console.log(
    "Remote-first (>=50% remote):",
    addressable.filter((a) => parseInt(a.remote_density) >= 50).length,
  );
  console.log(
    "High addressable density (>=50%):",
    addressable.filter((a) => parseInt(a.addr_density) >= 50).length,
  );
