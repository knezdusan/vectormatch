...
const MAINSTACK_KEYWORDS = [
  ...
  "ui engineer", "ui developer", "react native", "engineer", "developer",
  "programmer", "architect", "lead", "senior", "staff", "principal",
];
...
async function main() {
  ...
  for (const c of ashbyCompanies) {
    ...
    fingerprints.push({
      ...
      remote_density: jobs.length > 0 ? `${((remoteJobs / jobs.length) * 100).toFixed(0)}%` : "0%",
      global_density: jobs.length > 0 ? `${((globalRemoteJobs / jobs.length) * 100).toFixed(0)}%` : "0%",
      addressable_density: jobs.length > 0 ? `${((addressableJobs / jobs.length) * 100).toFixed(0)}%` : "0%",
  ...
  console.log(`Addressable rate: ${((addressableCount / fingerprints.length) * 100).toFixed(1)}%\n`);
  ...
  const topLocations = Object.entries(locationCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 15);
  console.table(topLocations.map(([loc, cnt]) => ({ location: loc, count: cnt })));

  // Title patterns
  const titleKeywords: Record<string, number> = {};
  for (const f of addressable) {
    const titles = f.sample_titles.toLowerCase();
    for (const kw of ["engineer", "developer", "senior", "staff", "lead", "fullstack",
      "frontend", "backend", "full stack", "full-stack", "react", "typescript",
      "node", "graphql", "ai", "ml", "data", "product", "design", "security",
      "devops", "platform", "infrastructure", "mobile", "ios", "android"]) {
  ...
  console.log("  1. Remote-first: high remote density (>50% of jobs are remote)");
  console.log("  2. Global remote: location is 'Remote' or 'Remote, US' or broad, not country-fenced");
  console.log("  3. Tech-focused: titles contain 'engineer', 'developer', 'senior', 'staff'");
  console.log("  4. Modern stack: react, typescript, node, graphql, fullstack");
  console.log("  5. Scale: 10-100+ active jobs (real hiring companies, not ghost boards)");
  console.log("");
  console.log("L2 source selection rubric:");
  console.log("  - Score each candidate source by how many companies match this fingerprint");
  console.log("  - Prioritize: VC-portfolio boards of modern-tech funds, AI/startup remote boards");
  console.log("  - Avoid: generic/SMB/multi-industry job boards (low addressable density)");
  console.log("  - Sample harvest before full ingestion: estimate addressable density from a sample");
