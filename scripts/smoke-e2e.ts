...
async function main() {
  ...
  log(
    "supply",
    supplyCount > 0 ? "PASS" : "FAIL",
    `${supplyCount} matchable jobs (active, global, unfenced, embedded)`,
  );
  ...
  log(
    "select-job",
    "PASS",
    `Selected: "${testJob.title}" (${testJob.ats_source}/${testJob.ats_slug})`,
    { id: testJob.id, tags: testJob.extracted_tags },
  );
  ...
  log(
    "gate-flags",
    flagsOk ? "PASS" : "FAIL",
    `fenced=${f.is_fenced} natsec=${f.is_natsec} qa=${f.is_qa} scope=${f.remote_scope} normalized=${f.normalized_at !== null}`,
  );
  ...
  log(
    "embedding",
    testJob.has_embedding ? "PASS" : "FAIL",
    `Job has embedding: ${testJob.has_embedding}`,
  );
  ...
  log(
    "personas",
    personaCount > 0 ? "PASS" : "FAIL",
    `${personaCount} active personas (${p.with_embedding} with embedding, ${p.with_tags} with tags)`,
  );
  ...
  try {
    ...
    log(
      "gate-1-2",
      gateResult.rows.length > 0 ? "PASS" : "WARN",
      `${gateResult.rows.length} candidates passed Gate 1+2`,
      gateResult.rows.map((r) => ({
    ...
    log("gate-1-2", "FAIL", `SQL error: ${(err as Error).message}`);
  }
  ...
  log(
    "match-queue",
    mq24h > 0 ? "PASS" : "WARN",
    `Total: ${mq.total}, Approved: ${mq.approved}, Pending: ${mq.pending}, Last 24h: ${mq24h}`,
  );
  ...
  log(
    "dashboard-query",
    dashCount > 0 ? "PASS" : "FAIL",
    `${dashCount} matches visible through serve-time gate filter`,
  );
  ...
  try {
    ...
    const healthCheck = await new Promise<{ status: number; ok: boolean }>(
      (resolve) => {
    ...
    log(
      "inngest-health",
      healthCheck.ok ? "PASS" : "FAIL",
      `Inngest health: ${healthCheck.status} ${healthCheck.ok ? "OK" : "UNREACHABLE"}`,
    );
  } catch {
    log(
      "inngest-health",
      "WARN",
      "Could not check Inngest health (no http module)",
    );
  }
  ...
  log(
    "ingestion-24h",
    jobs24h > 0 ? "PASS" : "WARN",
    `${jobs24h} jobs ingested in last 24h (${ing.normalized_24h} normalized)`,
  );
  ...
  log(
    "flow-ingestion",
    flowIngestCnt > 0 ? "PASS" : "FAIL",
    `${flowIngestCnt} jobs ingested in last 24h (scheduled pipeline must produce > 0)`,
  );
  ...
  log(
    "flow-normalization",
    flowNormCnt > 0 ? "PASS" : "FAIL",
    `${flowNormCnt} jobs normalized in last 24h`,
  );
  ...
  log(
    "flow-gate12",
    flowGate12Cnt > 0 ? "PASS" : "FAIL",
    `${flowGate12Cnt} new match candidates from Gate 1+2 in last 24h`,
  );
  ...
  log(
    "flow-gate3",
    flowGate3Cnt > 0 ? "PASS" : "FAIL",
    `${flowGate3Cnt} matches evaluated by Gate 3 in last 24h`,
  );
  ...
  log(
    "flow-dashboard",
    flowDashCnt > 0 ? "PASS" : "FAIL",
    `${flowDashCnt} new dashboard-visible matches in last 24h`,
  );
  ...
  console.log(
    `  FLOW: ${flowPassed}/${flowResults.length} stages passed, ${flowFailed} failed`,
  );
  if (flowFailed > 0) {
...
function report(): boolean {
  ...
  if (failed > 0) {
    ...
    if (flowFailed > 0) {
      console.log(
        "VERDICT: SMOKE TEST FAILED — pipeline is broken. Flow test shows the scheduled pipeline is not producing output. Do not deploy.",
      );
    } else {
      console.log(
        "VERDICT: SMOKE TEST FAILED — pipeline is broken (state test). Do not deploy.",
      );
    }
    process.exit(1);
  } else if (warned > 0) {
    ...
    console.log("");
    console.log(
      "VERDICT: SMOKE TEST PASSED WITH WARNINGS — pipeline is functional but not optimal.",
    );
    process.exit(0);
  } else {
    ...
    if (flowResults.length > 0) {
      console.log(
        "  FLOW TEST: All stages passed — scheduled pipeline is producing output organically.",
      );
    }
    process.exit(0);
  }
