...
// by Wappalyzer (Greenhouse, Lever, or Workable).
//
// ── Query result shape ───────────────────────────────────────────────────────
...
//   - ats_source: which ATS was detected ("greenhouse", "lever", or "workable")
//
...
export const bigQueryRowSchema = z
  .object({
    ...
    // Which ATS was detected by Wappalyzer ("greenhouse", "lever", or "workable").
    // Used as a hint for targeted slug probing (avoids trying all ATS APIs).
    ats_source: z.enum(["greenhouse", "lever", "workable"]),
  })
  .passthrough();
