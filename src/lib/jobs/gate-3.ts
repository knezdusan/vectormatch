...
OUTPUT RULES:
- Be balanced: approve if the tech stack and seniority align well, even if there are soft concerns (location, compliance, hybrid workplace). Only reject for HARD blockers (completely wrong tech stack, on-site when applicant is remote-only with no hybrid flexibility, blocklist tags, non-US country restrictions without compliance, EXPLICIT W-2-only US jobs for international contractors, work authorization requirements the applicant cannot satisfy). Soft concerns (US-only remote with w8ben compliance and ambiguous or no W-2 language, hybrid workplace) should be noted in the reasoning but should NOT cause rejection. When the applicant has w8ben/ic_global compliance and the job is US-only remote without explicit W-2/employee language, DEFAULT TO APPROVING with workAuthRiskFlag=true.
...
OUTPUT RULES:
- Be STRICT but fair: only reject for genuine hard blockers (wrong tech stack, on-site mismatch without hybrid flexibility, blocklist tags, non-US country restrictions without contractor compliance, EXPLICIT W-2-only US jobs for international contractors, work authorization requirements the applicant cannot satisfy). Soft concerns (hybrid workplace, US-only remote with w8ben compliance and ambiguous/no W-2 language) should be noted but should NOT cause rejection.
...
OUTPUT RULES:
- Be thorough: consider all criteria before deciding. A match doesn't require perfection — it requires plausibility. If the core tech stack aligns and there are no hard blockers, lean toward approving. Soft concerns (hybrid workplace, US-only remote with w8ben compliance and ambiguous/no W-2 language) should be noted but should NOT cause rejection. When the applicant has w8ben/ic_global compliance and the job is US-only remote without explicit W-2/employee language, DEFAULT TO APPROVING with workAuthRiskFlag=true.
- workAuthRiskFlag is a WARNING for the user, NOT a rejection reason. Never list "work authorization risk" in the blockers array. Never reject a job solely because of work authorization uncertainty — the flag exists so the user can verify before applying.
- If rejected, list ALL blockers in the blockers array.
- matchReasoning should be 1–3 sentences explaining the key factor(s) in your decision.
- matchConfidence reflects your certainty, not the match quality.`;
