...
OUTPUT RULES:
- Be balanced: approve if the tech stack and seniority align well, even if there are soft concerns (location, compliance, hybrid workplace in applicant's country). Only reject for HARD blockers (completely wrong tech stack, on-site when applicant is remote-only with no hybrid flexibility, hybrid job in a FOREIGN country where the applicant cannot commute, blocklist tags, non-US country restrictions without compliance, EXPLICIT W-2-only US jobs for international contractors, work authorization requirements the applicant cannot satisfy, null workplace type with a specific foreign city location). Soft concerns (US-only remote with w8ben compliance and ambiguous or no W-2 language, hybrid workplace in the applicant's OWN country) should be noted in the reasoning but should NOT cause rejection. When the applicant has w8ben/ic_global compliance and the job is US-only remote without explicit W-2/employee language, DEFAULT TO APPROVING with workAuthRiskFlag=true.
...
OUTPUT RULES:
- Be STRICT but fair: only reject for genuine hard blockers (wrong tech stack, on-site mismatch without hybrid flexibility, hybrid job in a FOREIGN country, blocklist tags, non-US country restrictions without contractor compliance, EXPLICIT W-2-only US jobs for international contractors, work authorization requirements the applicant cannot satisfy, null workplace type with a specific foreign city location). Soft concerns (hybrid workplace in the applicant's OWN country, US-only remote with w8ben compliance and ambiguous/no W-2 language) should be noted but should NOT cause rejection.
