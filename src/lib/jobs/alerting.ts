 ...
 * Called by the storage monitor Inngest function (every 6h).
 *
 * Uses `pg_database_size()` to check storage against `STORAGE_LIMIT_MB`.
 * The Neon API path was removed in D21 — the database is now self-hosted
 * on VPS Postgres, so `pg_database_size()` is the accurate source of truth.
 *
  ...
  if (percentage >= STORAGE_CRITICAL_THRESHOLD) {
    ...
    if (!(await hasActiveAlert("storage_critical"))) {
      await createAlert({
        ...
        severity: "critical",
        message: `Storage at ${sizeMb.toFixed(0)}MB / ${limitMb}MB (${(percentage * 100).toFixed(1)}%) — immediate action required`,
    ...
    if (!(await hasActiveAlert("storage_near_limit"))) {
      await createAlert({
        ...
        severity: "warning",
        message: `Storage at ${sizeMb.toFixed(0)}MB / ${limitMb}MB (${(percentage * 100).toFixed(1)}%) — batch refreshes are being skipped`,
        details: JSON.stringify({
          sizeMb,
          limitMb,
          percentage,
