...
5. **Inngest Orchestration Base** `[Status: Implemented]` — `app/api/inngest/route.ts` serve handler (Next.js App Router, `maxDuration: 300`), typed Inngest v4 client (`src/inngest/client.ts` with `VectorMatchEvents` catalog including Module C events `match/gate-3-evaluate`, `match/approved`, `match/bulk-reprocess`, and `persona/updated`), function registry (`src/inngest/functions.ts` with 48 functions — `jobIngestedHandler` for Module C normalization + Gate 1+2, `gate3Evaluator` for Module C LLM arbitration with A/B test prompt variants, `pendingQueueSweep` for stuck pending rows, `personaUpdatedHandler` for Gate 3 feedback loop, `batchPollTier` for G5 batch polling, `normalizationRetrySweep` for stuck-job recovery (limit 500/run), `pipelineHealthMonitor` for real-time pipeline alerting, `matchBulkReprocess` for retroactive matching of existing jobs, `matchRetrySweep` for daily re-matching of missed jobs, plus 39 Module B/A functions), barrel exports (`src/inngest/index.ts`), dev server scripts (`npm run inngest:dev`), MCP integration in `.devin/config.json`, and coding agent resources (`docs/reports/inngest-agent-resources.md`). (Completed June 2026, Gate 3 feedback loop + pending sweep added June 28 2026, pipeline health monitor added July 1 2026, bulk reprocess + retry sweep added July 1 2026.)
...
Module B is feature-complete with 1,324 unit/integration tests passing across 61 test files (includes corpus expansion: 28 CertStream tests, 27 Meta Ads tests, 23 Workable tests, plus all original seeder/poller tests). All 3 live testing layers have been completed against real ATS APIs and a real Neon dev branch (`module-b-testing`). The corpus expansion batch flush was executed against the production Neon database (June 29 2026) with 5,290 companies inserted.
