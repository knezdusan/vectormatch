...
5. **Inngest Orchestration Base** `[Status: Implemented]` — `app/api/inngest/route.ts` serve handler (Next.js App Router, `maxDuration: 300`), typed Inngest v4 client (`src/inngest/client.ts` with `VectorMatchEvents` catalog including Module C events `match/gate-3-evaluate`, `match/approved`, `match/bulk-reprocess`, and `persona/updated`), function registry (`src/inngest/functions.ts` with 52 functions — `jobIngestedHandler` for Module C normalization + Gate 0.5 pre-filter + Gate 1+2, `gate3Evaluator` for Module C LLM arbitration with A/B test prompt variants + work authorization directives, `pendingQueueSweep` for stuck pending rows, `personaUpdatedHandler` for Gate 3 feedback loop, `batchPollTier` for G5 batch polling, `normalizationRetrySweep` for stuck-job recovery (limit 500/run), `pipelineHealthMonitor` for real-time pipeline alerting, `matchBulkReprocess` for retroactive matching of existing jobs, `matchRetrySweep` for daily re-matching of missed jobs, `emergencyStoragePurge` for tiered emergency storage purge with WAL inflation protection, `hourlyStorageMonitor` for Neon synthetic storage monitoring, plus 41 Module B/A functions), barrel exports (`src/inngest/index.ts`), dev server scripts (`npm run inngest:dev`), MCP integration in `.devin/config.json`, and coding agent resources (`docs/reports/inngest-agent-resources.md`). (Completed June 2026, Gate 3 feedback loop + pending sweep added June 28 2026, pipeline health monitor added July 1 2026, bulk reprocess + retry sweep added July 1 2026, Gate 0.5 pre-filter + storage monitoring + emergency purge added July 4 2026, work authorization + Neon API integration + WAL inflation protection added July 4 2026.)
...
10. **3-Gate Routing Logic** `[Status: Implemented]` — Module C (Event-Driven Routing — The 3-Gate Funnel) fully implemented across 7 features (C0–C6):
    ...
    - All 1,623 Vitest tests pass (86 files), typecheck clean, biome clean. (Completed June 2026, yield analysis improvements June 28 2026, Sprint 8 matching pipeline fix July 1 2026, display score + negative signals July 2026.)
...
| `company` | Corpus Expansion (batch flush) | 5,290 (actual, June 29 2026) | One-time flush from 10 batch sources. Top source: Wayback CDX (4,163). YC Directory (374), Newsletter Archives (257), VC Portfolios (36), Cross-Pollination (11). Exceeded 5,000 target by 106%. |
| `company` (total) | All sources | ~10,000–20,000 | After 6–12 months of daily source seeding (D2-D13), the registry stabilizes. Dead companies are auto-disabled (`pollingEnabled = false`) but rows are NOT deleted (preserves discovery history). |
...
- **Step 3 (Gate 3 - High-Fidelity AI Arbitration)** `[Status: Implemented]`:
  ...
  - **Country-specific remote restrictions** (added June 28 2026): All prompt variants instruct the LLM to scan for geographic limitations (e.g., "remote (US only)") and reject country mismatches as hard blockers. Includes a dynamic compliance directive for W-8BEN/ic_global applicants that distinguishes contractor-friendly vs. W-2-only US postings.
...
## Hosting Infrastructure `[Status: Implemented — deployed and healthy, July 2026]`

Self-hosted PaaS (Hetzner Cloud CX33 + Coolify v4.1.2), chosen for flat solo-developer cost over Vercel's serverless model. Deployed on Hetzner CX33 (x86_64, Helsinki eu-central) with a 3-stage Dockerfile (Node 24-slim, standalone output, multi-arch). Neon Postgres in Frankfurt (aws-eu-central-1) for same-region query latency. Cloudflare free tier for edge protection with WAF rate-limiting on high-cost API endpoints. **Self-hosted Inngest** (Sprint 5, June 30 2026) deployed as a 3-container Docker Compose service (`inngest/inngest:v1.34.0` + `postgres:17` + `redis:7`) on the same Coolify infrastructure, accessible at `https://inngest.vectormatch.dev`. Replaced Inngest Cloud free plan (5 concurrent steps, 50K executions/month) with unlimited self-hosted execution.
