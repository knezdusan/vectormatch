...
10. **3-Gate Routing Logic** `[Status: Implemented]` — Module C (Event-Driven Routing — The 3-Gate Funnel) fully implemented across 7 features (C0–C6):
    ...
    - **C2 (Gate 1+2 SQL Router)**: `src/lib/jobs/gate-1-2.ts` with `runGateSQLRouter` — single-pass GIN overlap + HNSW cosine distance query with composite ordering (`overlap * w1 + (1 - distance) * w2 DESC`), `unnest`/`= ANY` overlap count (fixes `&` operator bug on `text[]`), `LIMIT 8` + `ON CONFLICT DO NOTHING`. Cross-posting dedup via `NOT EXISTS` subquery on `(ats_slug, title, persona_id)` — prevents ATS cross-posted duplicates (same job, different `external_job_id`) from creating duplicate matches. Wired into `jobIngestedHandler` step 5. 17 Vitest tests.
