...
Status: Final decision — implemented via self-hosted PaaS (Hetzner Cloud + Coolify). Deployed and running (healthy) as of July 14 2026. WordPress blog service added July 14 2026.
...
`src/app/layout.tsx` exports `metadata` with `metadataBase`, `title`, `description`, `openGraph`, `twitter`, `icons`, `manifest`, `alternates` (canonical only — the blog RSS feed was removed with the MDX blog; WordPress generates its own feed at `/blog/feed/`), and `robots`.
